...
# When creating a Google Form that prompts users for a short answer (or paragraph), it’s possible to enable response
# validation and require that the user’s input match a regular expression. For instance, you could require that a user input
# an email address with a regex like this one:
...
# call email() from validators library on input
# if truthy print Valid
# else print Invalid

# Time: O(n) email() scans input string, grows with length of address
# Space: O(1) no growing DS, fixed variables only
