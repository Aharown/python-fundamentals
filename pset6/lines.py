...
# # One way to measure the complexity of a program is to count its number of lines of code (LOC), excluding blank lines
# # and comments. For instance, a program like
...
# That’s not that many, so odds are the program isn’t that complex. Of course, just because a program (or even function)
# has more lines of code than another doesn’t necessarily mean it’s more complex. For instance, a function like
...
# to read! So lines of code should be taken with a grain of salt.
...
# optionally preceded by whitespace, is a comment. (A docstring should not be considered a comment.)
...
# Time: O(n) due to looping through file in count_lines()
...
# deal with sad paths too many args, not enough args, no args, arg doesn't exist
# deal with happy path, return file
...
# define function that counts LOC
# set accumulator to 0
# only accumulate if LOC is not prefixed with # and isn't empty
